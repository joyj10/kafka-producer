...
import com.study.kafkaproducer.vo.EffectOrNot;
import com.study.kafkaproducer.vo.PurchaseLog;
import com.study.kafkaproducer.vo.PurchaseLogOneProduct;
...
public class AdEvaluationService {
    // 광고 데이터는 중복 조인 필요 X --> table
    // 광고 이력이 먼저 들어옴
    // 구매 이력은 상품별로 들어 오지 않음 (복수개 상품 존재) -> contain
    // 광고 머문 시간이 10초 이상이여야 join 대상
    // 특정 가격 이상의 상품은 join 대상에서 제외
    // 광고 이력 : KTable(AdLog), 구매이력 : KStream(OrderLog)
    // filtering, 형 변환
    ...
    public void buildPipeline(StreamsBuilder streamsBuilder) {
        ...
        KTable<String, WatchingAdLog> adTable = streamsBuilder.stream("adLog", Consumed.with(Serdes.String(), watchingAdLogSerde))
        ...
        KStream<String, PurchaseLog> purchaseLogKStream = streamsBuilder.stream("purchaseLog", Consumed.with(Serdes.String(), purchaseLogSerde));
        ...
        purchaseLogKStream.foreach((k,v) -> {
                    ...
                    for (Map<String, String> prodInfo:v.getProductInfo())   {
                        ...
                        if (Integer.parseInt(prodInfo.get("price")) < 1000000) {
                            ...
                            producerService.sendJoinedMsg("purchaseLogOneProduct", tempVo);
        ...
        KTable<String, PurchaseLogOneProduct> purchaseLogOneProductKTable= streamsBuilder.stream("purchaseLogOneProduct", Consumed.with(Serdes.String(), purchaseLogOneProductSerdeSerde))
                // Stream의 key 지정
